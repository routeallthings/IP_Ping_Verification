...
sshenpasswordq = getpass.getpass('Enter the enable password for the devices: ')
...
def DEF_PINGCHECK(sshdevice):
	...
	#Start Connection
	try:
		sshnet_connect = ConnectHandler(device_type=sshdevicetype, ip=sshdeviceip, username=sshusernameq, password=sshpasswordq, secret=sshenpasswordq)
		...
		for pinglist in verificationcsv:
			...
			pingcheckarp = "ping " + pinglistip + " timeout 100 repeat 1"
			pingcheck = "ping " + pinglistip + " timeout 500 repeat 1"
